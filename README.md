only this config is verifed:
run with `nvim  -u ~/.config/nvim/repro.lua ~/.config/nvim/repro.lua`
incsearch by default, whenever you hit enter or esc, search ends, but you
can still use ;, to navagate previous set of matches. After you type any key
that is not ";" or "," , the ";" and "," are restored to orginal keys.
