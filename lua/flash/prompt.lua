...
function M.hide()
  ...
  vim.on_key(function(key, typed)
    if typed == ";" then
      M.jump_to_next_match(true)
    elseif typed == "," then
      M.jump_to_prev_match(true)
    else
      vim.on_key(nil, vim.api.nvim_create_namespace("flash;"))
      vim.api.nvim_buf_clear_namespace(0, vim.api.nvim_create_namespace("flash_match"), 0, -1)
